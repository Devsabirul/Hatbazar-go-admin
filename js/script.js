...
function initButtons() {
  ...
  document.getElementById('saveBkash')?.addEventListener('click', () => savePaymentSettings('bkash'));
  document.getElementById('saveNagad')?.addEventListener('click', () => savePaymentSettings('nagad'));

  // Delivery fee save button
  document.getElementById('saveDeliveryFee')?.addEventListener('click', saveDeliveryFee);
...
function navigateTo(page) { window.navigateTo(page); }
...
window.openOrderModal = async function (docId) {
  ...
  if (bodyEl) bodyEl.innerHTML = `<div style="text-align:center;padding:40px"><div class="spinner-sm"></div></div>`;
  ...
  try {
    ...
    if (titleEl) titleEl.textContent = `Order — ${o.order_id || o.id.slice(0, 8)}`;
    ...
    if (bodyEl) bodyEl.innerHTML = `<p style="color:var(--danger)">Failed to load order.</p>`;
  }
};
...
async function loadRiders() {
  ...
  if (tbody) tbody.innerHTML = `<tr><td colspan="5" class="td-loading"><div class="spinner-sm"></div></td></tr>`;
  ...
  try {
    ...
    console.error('Load riders error:', err);
    if (tbody) tbody.innerHTML = `<tr><td colspan="5" class="td-empty" style="color:var(--danger)">Failed to load riders.</td></tr>`;
  }
}
...
window.openRiderModal = async function (docId) {
  ...
  try {
    ...
    if (!snap.exists()) { closeModal('riderModalOverlay'); return; }
    ...
    if (bodyEl) bodyEl.innerHTML = `<p style="color:var(--danger)">Failed to load rider.</p>`;
  }
};
...
async function loadStores() {
  ...
  if (tbody) tbody.innerHTML = `<tr><td colspan="6" class="td-loading"><div class="spinner-sm"></div></td></tr>`;
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = `<tr><td colspan="6" class="td-empty" style="color:var(--danger)">Failed to load stores.</td></tr>`;
  }
}
...
window.openStoreModal = async function (docId) {
  ...
  try {
    const snap = await getDoc(doc(db, 'users', docId));
    if (!snap.exists()) { closeModal('storeModalOverlay'); return; }
    ...
    if (bodyEl) bodyEl.innerHTML = `<p style="color:var(--danger)">Failed to load store.</p>`;
  }
};
...
async function loadProducts() {
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = `<tr><td colspan="6" class="td-empty" style="color:var(--danger)">Failed to load products.</td></tr>`;
  }
}
...
async function loadUsers() {
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = `<tr><td colspan="5" class="td-empty" style="color:var(--danger)">Failed to load users.</td></tr>`;
  }
}
...
async function loadDeliveryFee() {
  try {
    ...
    if (!snap.empty) {
      const data = snap.docs[0].data();
      const fee = data.fee || '';
      // HTML only has id="deliveryFeeInput" (no separate display element)
...
function initSearchFilters() {
  ...
  if (orderTabs) {
    orderTabs.addEventListener('click', (e) => {
      ...
      if (!tab) return;
      orderTabs.querySelectorAll('.filter-tab').forEach(t => t.classList.remove('active'));
